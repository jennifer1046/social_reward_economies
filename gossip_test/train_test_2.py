...
def get_active_agents(agents_list):
    # Test 1: Force all agents to be active at all timesteps
    active_list = [agent.num for agent in agents_list]
    inactive_list = []
...
def training_loop(num_agents, max_state_num, name, arguments, k=1.0, th_factor=0.5, discount=0.99, adjustment_factor=1.0,
    ...
    # For max_state_num=0 (single state), we need state_size=1 to represent state [0]
    # For max_state_num>0, calculate bits needed: ceil(log2(max_state_num+1))
    if max_state_num == 0:
        state_size = 1
    else:
    ...
    for agn in range(num_agents):
        ...
        # Test 2: Different reward functions for each agent
        # Agent 0: evaluates all actions as providing utility 1.0
        # Agent 1: evaluates all actions as providing utility 2.0
        # Agent 2: evaluates all actions as providing utility 3.0
        # This gives different reputations: R_0 = 2+3 = 5, R_1 = 1+3 = 4, R_2 = 1+2 = 3
        agents_list[agn].reward_function = norm_env.constant_reward_table(
            state_size, possible_action_size, value=float(agn + 1)
        )
    ...
    for i in range(timesteps):
        ...
        # Debug: Print utility values occasionally to verify reward functions are different
        if i < 5 and len(active_list) >= 2:
            print(f"Timestep {i}: Agent 0 evaluates action from Agent 1: {agents_list[0].get_utility(state, crowd_actions[1]):.3f}")
            print(f"Timestep {i}: Agent 1 evaluates action from Agent 0: {agents_list[1].get_utility(state, crowd_actions[0]):.3f}")
            if len(active_list) >= 3:
                print(f"Timestep {i}: Agent 2 evaluates action from Agent 0: {agents_list[2].get_utility(state, crowd_actions[0]):.3f}")
        ...
        if i > gossiping_start:
            # Debug: Print PR values before gossiping to see if they're different
            if i == gossiping_start + 1 or (i % 1000 == 0 and i < gossiping_start + 5000):
                print(f"\nTimestep {i}: PR values before gossiping:")
                for agent_j in range(num_agents):
                    print(f"  Agent {agent_j} PR: {[agents_list[agent_j].PR[k] for k in range(num_agents)]}")
                print(f"Timestep {i}: R values before gossiping:")
                for agent_j in range(num_agents):
                    print(f"  Agent {agent_j} R: {[agents_list[agent_j].R[k] for k in range(num_agents)]}")
        ...
        if i % update_interval == 0 and i > 0:
            ...
            for ii in agents_list:
                sums = 0
                # True reputation: R_i = Σ_{j≠i} R_j[i] (sum excluding self, matching formula)
                for jj in agents_list:
                    if jj.num != ii.num:  # Exclude agent ii's self-estimate
                        sums += jj.R[ii.num]
                # Apply gamma (reputation_factor) to match formula: R_i = γ Σ_{j≠i} U_j(π_i)
                reputations[ii.num].append(sums * arguments.get("reputation_factor", 1.0))
                sums1 = 0
                for jj in agents_list:
                    sums1 += jj.PR[ii.num]
    ...
    print("\n" + "=" * 60)
    print("TEST 2: Different Reward Functions - Reputation Summary")
    print("=" * 60)
    print(f"Configuration:")
    print(f"  - Number of agents = {num_agents}")
    print(f"  - Agent 0 reward function: evaluates all actions as utility 1.0")
    print(f"  - Agent 1 reward function: evaluates all actions as utility 2.0")
    print(f"  - Agent 2 reward function: evaluates all actions as utility 3.0")
    print(f"  - Gamma (γ) = {arguments.get('reputation_factor', 1.0)}")
    print(f"\nExpected Reputations (R_i = γ Σ_ U_j(π_i)):")
    print(f"  - R_0 = u_1(π_0) + u_2(π_0) = 2.0 + 3.0 = 5.0")
    print(f"  - R_1 = u_0(π_1) + u_2(π_1) = 1.0 + 3.0 = 4.0")
    print(f"  - R_2 = u_0(π_2) + u_1(π_2) = 1.0 + 2.0 = 3.0")
    
    print(f"\nFinal Reputation Values (Sum of estimates - matches formula):")
    gamma = arguments.get('reputation_factor', 1.0)
    for agent_i in range(num_agents):
        # Sum all other agents' estimates (excluding self)
        sum_reputation = sum(agents_list[j].R[agent_i] for j in range(num_agents) if j != agent_i)
        # Expected value
        expected = sum((j + 1) for j in range(num_agents) if j != agent_i) * gamma
        diff = abs(sum_reputation - expected)
        print(f"  Agent {agent_i}: R = {sum_reputation:.6f} (expected: {expected:.6f}, diff: {diff:.6f})")
    
    print(f"\nIndividual Reputation Estimates (R_j[i] - what agent j thinks about agent i):")
    for agent_i in range(num_agents):
        print(f"  Agent {agent_i}'s reputation as estimated by others:")
        for agent_j in range(num_agents):
            if agent_j != agent_i:
                print(f"    Agent {agent_j} estimates R_{agent_j}[{agent_i}] = {agents_list[agent_j].R[agent_i]:.6f}")
        # Also show self-estimate (should be 0 or not used)
        print(f"    Agent {agent_i} self-estimate R_{agent_i}[{agent_i}] = {agents_list[agent_i].R[agent_i]:.6f} (not used in formula)")
    
    print(f"\nAveraged Reputation Estimates (for reference):")
    for agent_i in range(num_agents):
        avg_reputation = sum(agents_list[j].R[agent_i] for j in range(num_agents)) / num_agents
        print(f"  Agent {agent_i}: Average R_j[{agent_i}] = {avg_reputation:.6f}")
    
    print(f"\nPR (Personal Reputation) Values - These should reflect different utilities:")
    for agent_j in range(num_agents):
        print(f"  Agent {agent_j} PR values: {[agents_list[agent_j].PR[k] for k in range(num_agents)]}")
        print(f"    (Agent {agent_j} evaluates actions as utility {agent_j + 1}, so PR should accumulate differently)")
    
    print(f"\nNote: If all R values are the same, the gossiping mechanism is averaging everything.")
    print(f"      The gossiping averages all agents' R estimates, which causes convergence.")
    print(f"      PR values should be different, but R gets averaged during gossiping.")
    print("=" * 60 + "\n")
...
def new_plot_embed_withx(agents_list, xseries, yseries, root, name, title, top_agent):
    plt.figure(name + title, figsize=(12, 7))
    ax = plt.subplot(111)
    box = ax.get_position()
    # For Test 1: Show all agents in legend since they should all converge to the same value
    # Use different line styles and alpha to make overlapping lines visible
    import matplotlib.cm as cm
    colors = cm.get_cmap('tab10', len(agents_list))
    linestyles = ['-', '--', '-.', ':', '-', '--', '-.', ':', '-', '--']
    
    # Ensure all agents are plotted - iterate through all agents in the list
    plotted_agents = []
    for idx, agent in enumerate(agents_list):
        agent_num = agent.num
        # Verify data exists for this agent
        if agent_num < len(yseries) and len(yseries[agent_num]) > 0:
            # Use different colors and line styles for visibility
            ax.plot(xseries, yseries[agent_num], 
                    label="Agent " + str(agent_num),
                    color=colors(idx),
                    linestyle=linestyles[idx % len(linestyles)],
                    alpha=0.8,
                    linewidth=1.5)
            plotted_agents.append(agent_num)
        else:
            print(f"Warning: No data found for Agent {agent_num} in {name}")
    
    # Set position and add legend
    ax.set_position([box.x0, box.y0, box.width * 0.7, box.height])
    if len(plotted_agents) > 0:
        ax.legend(loc='center left', bbox_to_anchor=(1, 0.5), fontsize=9, ncol=1)
    else:
        print(f"Warning: No agents plotted for {name}")
    ...
    if "reputations" in name.lower():
        print(f"\nReputation plot summary: Plotted {len(plotted_agents)} agents: {plotted_agents}")


#state_size = 3
max_state_num = 0  # Test 2: 1 state (state 0)
# state_size is calculated inside training_loop function
# For max_state_num=0, state_size will be 1
name = "test"
agents_list = []
...
scale_factor = 1
rep_factor = 1.0  # Test 1: gamma = 1.0
alpha = 0.00005
...
# - 3 agents, 1 state, 1 action
# - Each agent j has reward function that evaluates all actions as providing utility (j+1)
#   - Agent 0: reward = 1.0
#   - Agent 1: reward = 2.0
#   - Agent 2: reward = 3.0
# - gamma = 1.0
# - All agents participate in all timesteps
# - Expected reputations:
#   - R_0 = u_1(π_0) + u_2(π_0) = 2.0 + 3.0 = 5.0
#   - R_1 = u_0(π_1) + u_2(π_1) = 1.0 + 3.0 = 4.0
#   - R_2 = u_0(π_2) + u_1(π_2) = 1.0 + 2.0 = 3.0

for splits in [(-1.25, 2.25)]:  # , (-29, 30)]:
        for rep_factor in [1.0]:  # Test 2: gamma = 1.0
            for scale_factor in [0.1]:
                for threshold in [48]:
                    for epsilon in [1]:
                        ...
                        args["reputation_factor"] = base_rep * rep_factor  # = 1.0
                        # args["top_reward"] = splits[1]  # Not needed for constant reward
                        # args["bottom_reward"] = splits[0]  # Not needed for constant reward
                        args["threshold"] = threshold
                        args["b0"] = base_b0 * splits[1] * 4
                        args["cons"] = 3000
                        args["epsilon"] = epsilon
                        dyn_change = False  # Test 2: static (all agents always active)
                        # timesteps1 = 15000 #int(cons * 3.5)
                        sync_label = "static"
                        if dyn_change:
                            sync_label = "async"
                        training_loop(3, max_state_num,  # Test 2: 3 agents, 1 state (max_state_num=0)
                                      "test2_different_reward_functions_3_agents",
