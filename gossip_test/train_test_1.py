...
def training_loop(num_agents, max_state_num, name, arguments, k=1.0, th_factor=0.5, discount=0.99, adjustment_factor=1.0,
    ...
    for agn in range(num_agents):
        ...
        # Test 1: All utilities u_j(s, π_i(s)) = 1 for all agents, states, and actions
...
# - All utilities u_j(s, π_i(s)) = 1 for all agents, states, and actions
# - gamma = 1.0
# - Number of agents = 10
# - Expected reputation: R_i = (10-1) × 1 × 1 = 9 for all agents i
# - All agents participate in all timesteps

for splits in [(-1.25, 2.25)]:  # , (-29, 30)]:
        for rep_factor in [1.0]:  # Test 1: gamma = 1.0
            for scale_factor in [0.1]:
                for threshold in [48]:
                    for epsilon in [1]:
                        ...
                        dyn_change = False  # Test 1: static (all agents always active)
                        # timesteps1 = 15000 #int(cons * 3.5)
                        sync_label = "static"
                        if dyn_change:
                            sync_label = "async"
                        training_loop(10, max_state_num,  # Test 1: 10 agents
                                      "gossip_network_test_1",
