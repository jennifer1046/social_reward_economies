...
class NormAgent:
    ...
    def adjust_rate(self):
        ...
        if self.selfless == 1:
            usable_beta = max(self.influencer_network.beta, self.R[self.num] * self.reputation_factor, 0)
            self.rate = self.rate_from_alpha(usable_beta, 0)
        else:
            if self.target_influencer != -1 and self.followers == 0:
                usable_beta = max(self.selfish_beta, self.R[self.target_influencer] * self.reputation_factor, 0)
                self.rate = self.rate_from_alpha(usable_beta, 0)
            else:
