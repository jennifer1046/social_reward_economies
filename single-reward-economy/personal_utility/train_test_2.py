...
def get_test2_state():
    """
    Test 2: One State, Multiple Actions
    
    State space: S = {0} with probability p(0) = 1
    Action space: A = {0, 1}
    Utilities: u_i(0,0) = 0, u_i(0,1) = 1
    
    Expected: All agents should choose action 1 (policy π_i(0) = 1)
    """
    # Single state s = 0
    state = np.array([0])
...
def training_loop(num_agents, max_state_num, name, arguments, k=1.0, adjustment_factor=1.0, timesteps=50000):
    ...
    sample_state, sample_action_space = get_test2_state()
    ...
    for agn in range(num_agents):
        ...
        agents_list[agn].reward_function = norm_env.test2_reward_table(state_size, possible_action_size)
    ...
    for i in range(timesteps):
        ...
        state, actions = get_test2_state()  # Choose a random state
        ...
        if i % update_interval == 0 and i > 0:
            ...
            # Sample actions from current policy for state 0
            state, actions = get_test2_state()
    ...
    if len(actions_chosen[0]) > 0:
        ...
        print(f"  Expected: All agents should choose action 1 (utility = 1.0)")
...
def plot_actions_chosen(agents_list, xseries, actions_chosen, root):
    ...
    For Test 2: action 0 should give utility 0, action 1 should give utility 1.
    Expected: All agents should converge to choosing action 1.
    """
    ...
    ax.axhline(y=0, color='r', linestyle='--', alpha=0.3, label='Action 0 (u=0)')
    ax.axhline(y=1, color='g', linestyle='--', alpha=0.3, label='Action 1 (u=1)')
    
    ax.set_position([box.x0, box.y0, box.width * 0.8, box.height])
    ax.set_xlabel("Timestep")
    ax.set_ylabel("Action Chosen")
    ax.set_title("Actions Chosen by Agents Over Time\n(Expected: All agents converge to action 1)")
    ax.set_ylim(-0.2, 1.2)
    ax.set_yticks([0, 1])
    ...
    ax = plt.subplot(111)
    ax.plot(xseries, action0_counts, 'r-o', label='Action 0', markersize=3, alpha=0.7)
    ax.plot(xseries, action1_counts, 'g-s', label='Action 1', markersize=3, alpha=0.7)
    ax.set_xlabel("Timestep")
    ax.set_ylabel("Number of Agents")
    ax.set_title("Distribution of Actions Over Time\n(Expected: All agents choose action 1)")
    ax.legend()
    ax.grid(True, alpha=0.3)
...
training_loop(
        num_agents=10,
        max_state_num=0,  # Single state: S = {0}
        name="personal_utility_test_2",
        arguments=args,
        k=1.0,
