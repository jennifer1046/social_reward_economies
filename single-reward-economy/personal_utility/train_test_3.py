...
def compute_overall_policy(max_state, agents_list, p_network_list, i_network_list, state_probs=None):
    ...
    print(f"Expected utility (Test 3): 0.6 * 1 + 0.4 * 3 = {0.6 * 1 + 0.4 * 3}")
...
def get_test3_state():
    """
    Test 3: Multiple States, One Action
    
    State space: S = {0, 1} with probabilities p(0) = 0.6, p(1) = 0.4
    Action space: A = {0}
    Utilities: u_i(0, 0) = 1, u_i(1, 0) = 3
    Expected utility: 0.6 * 1 + 0.4 * 3 = 1.8
    """
    # Sample state according to probabilities: p(0) = 0.6, p(1) = 0.4
...
def training_loop(num_agents, max_state_num, name, arguments, k=1.0, adjustment_factor=1.0, timesteps=50000):
    # For Test 3: max_state_num=1 (states 0 and 1)
    ...
    sample_state, sample_action_space = get_test3_state()
    possible_action_size = len(sample_action_space)
    
    # State probabilities for Test 3: p(0) = 0.6, p(1) = 0.4
    state_probs = [0.6, 0.4]  # Probabilities for states 0 and 1
    ...
    for agn in range(num_agents):
        ...
        agents_list[agn].reward_function = norm_env.test3_reward_table(state_size, possible_action_size)
    ...
    if True:
        ...
        # Track state distribution to verify p(0) = 0.6, p(1) = 0.4
    ...
    for i in range(timesteps):
        ...
        state, actions = get_test3_state()  # Sample state according to p(0)=0.6, p(1)=0.4
        ...
        if i % update_interval == 0 and i > 0:
            ...
            # Use state probabilities: p(0) = 0.6, p(1) = 0.4
            ...
            if total_states_seen > 0:
                ...
                print(f"  State distribution: p(0)={dist[0]:.3f}, p(1)={dist[1]:.3f} (expected: 0.600, 0.400)")
            ...
            powers = np.arange(bits - 1, -1, -1)
            pos_states = list((numbers >> powers) & 1)
            print(f"  Policy probabilities (should be 1.0 for action 0 in both states):")
            for statenum, statevec in enumerate(pos_states):
            ...
            if len(self_policy_e_util[0]) > 0:
                ...
                print(f"  Expected utility from policy: {current_expected_util:.4f} (target: 1.8000)")
                print(f"  Difference from target: {abs(current_expected_util - 1.8):.4f}")
    ...
    if total_final > 0:
        ...
        print(f"  Expected: State 0 = 60%, State 1 = 40%")
    ...
    for statenum, statevec in enumerate(pos_states):
        ...
        output = p_network_list[0].get_function_output(np.array(statevec))
        print(f"  State {statenum}: action probabilities = {[f'{p:.4f}' for p in output]}")
        print(f"    Expected: [1.0] for action 0 (only action available)")
...
def plot_state_distribution(xseries, state_distribution_over_time, root):
    ...
    ax.axhline(y=0.6, color='b', linestyle='--', alpha=0.3, label='Expected p(0) = 0.6')
    ax.axhline(y=0.4, color='r', linestyle='--', alpha=0.3, label='Expected p(1) = 0.4')
...
def plot_policy_probabilities(agents_list, xseries, behaviours, max_state_num, root):
    ...
    For Test 3: Should show probability 1.0 for action 0 in both states.
    """
    bits = max(1, max_state_num.bit_length())
    numbers = np.arange(max_state_num + 1)[:, np.newaxis]
    powers = np.arange(bits - 1, -1, -1)
        ...
        for agent in agents_list:
            if len(behaviours[agent.num]) > statenum:
                if len(behaviours[agent.num][statenum]) > 0:
                    # Get probabilities for action 0 (only action in Test 3)
                    action0_probs = [b[0] if len(b) > 0 else 0 for b in behaviours[agent.num][statenum]]
                    ax.plot(xseries, action0_probs, label=f"Agent {agent.num}", alpha=0.7, linewidth=1)
        
        ax.axhline(y=1.0, color='g', linestyle='--', alpha=0.5, label='Expected (1.0)')
        ax.set_xlabel("Timestep")
        ax.set_ylabel("Probability of Action 0")
        ax.set_title(f"Policy Probabilities for State {statenum}\n(Expected: 1.0 for action 0)")
        ax.set_ylim(-0.1, 1.1)
        ax.legend()
        ax.grid(True, alpha=0.3)
...
training_loop(
        ...
        name="personal_utility_test_3_500000_steps_0.0005_lr",
        arguments=args,
        k=1.0,
