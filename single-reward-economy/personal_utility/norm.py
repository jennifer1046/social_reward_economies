...
class NormEnv:
    ...
    def test2_reward_table(self, state_size, action_space):
  
        reward = {}

        # Only state 0 is meaningful; others (if any) get zero reward
        for s in range(2 ** state_size):
            reward[s] = [0.0] * action_space
    ...
    def test3_reward_table(self, state_size, action_space):
        """
        ...
        # Create rewards for all possible states
        for s in range(2 ** state_size):
            reward[s] = [0.0] * action_space
        
        # Set utilities: u(0, 0) = 1, u(1, 0) = 3
        reward[0][0] = 1.0  # State 0, action 0
        reward[1][0] = 3.0  # State 1, action 0
        
        return reward
    
    def test4_reward_table(self, state_size, action_space):
        """
        ...
        # Set utilities according to Test 4 specification
        reward[0][0] = 1.0  # State 0, action 0
        reward[0][1] = 0.0  # State 0, action 1
        reward[1][0] = 0.0  # State 1, action 0
        reward[1][1] = 2.0  # State 1, action 1
